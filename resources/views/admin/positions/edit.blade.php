@extends('layouts.admin', ['title' => 'Edit Jabatan', 'header' => 'Edit Data Jabatan'])
...
<div class="max-w-2xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Data Jabatan</h1>
            <p class="text-xs text-slate-500 mt-0.5">Perbarui nama, divisi, atau jenjang jabatan {{ $position->name }}</p>
        </div>
    ...
    <!-- Form -->
    <form action="{{ route('admin.positions.update', $position->id) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-4">
        @csrf
        @method('PUT')
            ...
            <div>
                ...
                <select name="level" id="level" required
                    ...
                    @foreach($levels as $lvl)
                        <option value="{{ $lvl }}" {{ old('level', $position->level) == $lvl ? 'selected' : '' }}>
                            {{ $lvl }}
                        </option>
                    @endforeach
                </select>
