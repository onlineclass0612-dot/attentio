@extends('layouts.admin', ['title' => 'Edit Kantor', 'header' => 'Edit Lokasi & Geofence'])
...
<div class="max-w-3xl mx-auto space-y-6" x-data="branchPicker()">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-xl font-bold text-slate-900 tracking-tight">Edit Lokasi: {{ $branch->name }}</h1>
            <p class="text-xs text-slate-500 mt-0.5">Perbarui koordinat titik pusat dan radius geofence</p>
        </div>
    ...
    <form action="{{ route('admin.branches.update', $branch->id) }}" method="POST" class="bg-white rounded-2xl border border-slate-200/80 p-6 shadow-sm space-y-5">
        @csrf
        @method('PUT')
        ...
        <div>
            ...
            <textarea name="address" rows="2"
                      class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-600">{{ old('address', $branch->address) }}</textarea>
        </div>
        ...
        <div>
            ...
            <p class="text-[11px] text-slate-400 mt-1">Klik di mana saja pada peta atau geser pin untuk memperbarui koordinat.</p>
        </div>
        ...
        <div>
            ...
            <select name="is_active" required class="w-full px-3.5 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-900 focus:bg-white focus:ring-2 focus:ring-blue-600">
                <option value="1" {{ $branch->is_active ? 'selected' : '' }}>Aktif</option>
                <option value="0" {{ !$branch->is_active ? 'selected' : '' }}>Non-Aktif</option>
            </select>
        </div>
